...
class LLMService:
    ...
    def _extract_tasks_heuristically(self, transcript: str) -> list[dict[str, Any]]:
        ...
        """
        patterns = (
            r"(?P<assignee>[А-ЯA-ZЁ][а-яa-zё]+)[,:]?\s+(?P<title>(?:сделай|сделать|подготовь|подготовить|проверь|проверить|доделай|доделать|создай|создать|исправь|исправить|перенеси|перенести)\b[^.!?\n]*)",
            r"(?P<title>(?:нужно|надо|задача|поручение)\s+[^.!?\n]*?)(?:\s+ответственный[:\s]+(?P<assignee>[А-ЯA-ZЁ][а-яa-zё]+))?",
        )
                ...
                tasks.append(
                    {
                        "блок": "Новые",
                        "задача": title,
                        "ответственный": match.groupdict().get("assignee"),
                        "срок": deadline_match.group(0) if deadline_match else None,
