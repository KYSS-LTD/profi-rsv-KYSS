...
@router.get("", response_model=list[V2TaskResponse], summary="List tasks", description="List Komandus source-of-truth tasks scoped to the current organization.")
...
def create_task(payload: V2TaskCreate, current_user=Depends(RoleChecker(Role.MANAGER, Role.PRODUCT_MANAGER)), db: Session = Depends(get_db)):
...
def move_task(task_id: UUID, payload: TaskStatusUpdate, current_user=Depends(RoleChecker(Role.MANAGER, Role.PRODUCT_MANAGER, Role.EMPLOYEE)), db: Session = Depends(get_db)):
