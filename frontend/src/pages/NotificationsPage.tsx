...
const items = [
  ...
  { icon: <PlugZap />, title: 'Ошибка интеграции', text: 'YouGile или OpenAI недоступны.' },
  { icon: <CheckCircle2 />, title: 'Отказ сотрудника', text: 'Причина отказа сохраняется для аналитики.' },
];
export function NotificationsPage() { return <><PageHeader eyebrow="Notification Center" title="Уведомления" description="Колокольчик и центр событий для задач, отказов, просрочек, интеграций и подключений сотрудников." /><section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{items.map((item) => <Card key={item.title}><div className="mb-4 flex h-11 w-11 items-center justify-center rounded-2xl bg-stone-100 text-stone-700">{item.icon}</div><h2 className="font-semibold text-stone-950">{item.title}</h2><p className="mt-2 text-sm leading-6 text-stone-500">{item.text}</p></Card>)}</section></>; }
