...
export function AIInboxPage() {
  ...
  return <><PageHeader eyebrow="Очередь AI" title="Входящие AI" description="Очередь задач, которые AI извлёк из Telegram. 85%+ отправляются сотруднику автоматически, остальные требуют проверки." />
    <section className="grid gap-4 xl:grid-cols-2">{inbox.map((task) => <Card key={task.id}>
      <div className="mb-4 flex items-start justify-between gap-4"><div className="flex items-center gap-3"><div className="rounded-2xl bg-stone-900 p-3 text-white"><Bot className="h-5 w-5" /></div><div><h2 className="font-semibold text-stone-950">{task.title}</h2><p className="text-sm text-stone-500">Уверенность {Math.round((task.llm_confidence ?? 0) * 100)}%</p></div></div><Badge tone={(task.llm_confidence ?? 0) >= 0.85 ? 'green' : 'amber'}>{(task.llm_confidence ?? 0) >= 0.85 ? 'авто' : 'на проверку'}</Badge></div>
