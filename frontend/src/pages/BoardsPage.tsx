...
const lifecycle = ['Open → OPEN', 'In Progress → IN_PROGRESS', 'Done → DONE', 'Cancelled → CANCELLED'];
...
export function BoardsPage() {
  ...
  const cards = [
    ...
    { title: 'OpenAI', icon: <Bot />, status: 'Настроен через backend env', last: 'по запросу AI', errors: 'Нет' },
  ];
  return <><PageHeader eyebrow="Интеграции" title="Интеграции" description="Telegram слушает только зарегистрированные через /register рабочие чаты и топики, AI извлекает задачи, YouGile получает подтвержденные карточки." />
