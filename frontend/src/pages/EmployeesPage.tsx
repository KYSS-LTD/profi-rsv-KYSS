...
import { AtSign, BadgeCheck, Copy, MessageCircle, Plus, RotateCcw, Shield, Trash2, UserRoundCheck } from 'lucide-react';
import { FormEvent, ReactNode, useMemo, useState } from 'react';
import { Employee, Role } from '../entities/saas/types';
import { activateEmployee, createEmployee, deactivateEmployee, deleteEmployee, getEmployees, restoreEmployee } from '../shared/api/employeesV2';
...
export function EmployeesPage() {
  ...
  const deactivateMutation = useMutation({ mutationFn: deactivateEmployee, onSuccess: invalidate });
  const activateMutation = useMutation({ mutationFn: activateEmployee, onSuccess: invalidate });
  const restoreMutation = useMutation({ mutationFn: restoreEmployee, onSuccess: invalidate });
  const deleteMutation = useMutation({ mutationFn: deleteEmployee, onSuccess: invalidate });
  ...
  return (
    <>
      <PageHeader eyebrow="Команда" title="Команда" description="Карточки сотрудников: роль, отдел, команда, Telegram username/id, дата подключения и безопасная привязка через /start." actions={<Button form="employee-form" type="submit" disabled={createMutation.isPending}><Plus className="h-4 w-4" />Добавить</Button>} />
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[420px_1fr]">
        <Card className="h-fit">
          ...
          <p className="mt-2 text-sm leading-6 text-stone-500">Telegram ID не вводится вручную: менеджер указывает @username, а бот автоматически связывает аккаунт после /start.</p>
          <form id="employee-form" className="mt-5 space-y-4" onSubmit={onCreate}>
            ...
            <Select value={role} onChange={(e) => setRole(e.target.value as Role)}>{roles.map((item) => <option key={item} value={item}>{roleLabel[item]}</option>)}</Select>
            <Select value={departmentId} onChange={(e) => { setDepartmentId(e.target.value); setTeamId(''); }}><option value="">Отдел</option>{departments.map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</Select>
            <Select value={teamId} onChange={(e) => setTeamId(e.target.value)}><option value="">Команда</option>{teams.filter((t) => !departmentId || t.department_id === departmentId).map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</Select>
            <Input value={telegramUsername} onChange={(e) => setTelegramUsername(e.target.value)} placeholder="@ivan_petrov" />
            {createMutation.error && <p className="rounded-xl bg-red-50 px-3 py-2 text-sm text-red-700">{createMutation.error.message}</p>}
          </form>
          {invite && <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900"><p className="whitespace-pre-line">{invite}</p><Button className="mt-3" variant="secondary" onClick={() => navigator.clipboard.writeText(invite)}><Copy className="h-4 w-4" />Скопировать приглашение</Button></div>}
        </Card>
        <div className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">{employees.map((employee) => <EmployeeCard key={employee.id} employee={employee} department={departmentName(employee.department_id)} team={teamName(employee.team_id)} onActivate={() => activateMutation.mutate(employee.id)} onDeactivate={() => deactivateMutation.mutate(employee.id)} onRestore={() => restoreMutation.mutate(employee.id)} onDelete={() => window.confirm('Деактивировать сотрудника?') && deleteMutation.mutate(employee.id)} />)}</div>
        </div>
      </section>
    </>
  );
...
function EmployeeCard({ employee, department, team, onActivate, onDeactivate, onRestore, onDelete }: { employee: Employee; department: string; team: string; onActivate: () => void; onDeactivate: () => void; onRestore: () => void; onDelete: () => void }) {
...
function telegramStatus(employee: Employee): { label: string; tone: 'green' | 'amber' | 'red' | 'neutral' } {
  if (employee.telegram_status === 'CONNECTED' && employee.telegram_id) return { label: '🟢 Telegram подключен', tone: 'green' };
  if (employee.telegram_status === 'NOT_FOUND') return { label: '🔴 Не найден', tone: 'red' };
  return { label: '🟡 Ожидает подключения', tone: 'amber' };
}
